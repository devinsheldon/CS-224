...
void applyFilterToRow(unsigned char* row, int width, int isGrayscale) {
  ...
  for(int i = 0; i < width; i++){
    ...
    tmp = tmp + 3*i;  // <-----------------------put before or after applyFilterToPixel() ??
...
void applyFilterToPixelArray(unsigned char* pixelArray, int width, int height, int isGrayscale) {
  ...
  padding = (width) % 4; //say width = 2, a row is 6 bytes. 2%4=2 --> 2 bytes of padding --> 6+2=8, 8 is a mutiple of 4
                  //say width = 13, a row of 39 bytes. 13%4= --> 1 bytes of padding --> 39+1=40, 40 is a multiple of 4
#ifdef DEBUG
  printf("padding = %d\n", padding);
#endif
  ...
  for(int i = 0; i < height; i++){ //applyFilterToRow needs to be passed the pointer to the beginning of each row
    applyFilterToRow((pixelArray + offset), width, isGrayscale); 
    offset += (width*3) + padding;//pixelArray will always point to the first byte of the image; just add a bigger and bigger offset each time
  }
}
